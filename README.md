...
<details>
  ...
  ___
  + Format of the config file: <br><br>
    ...
    Parameters needed:<br>
      ...
      - SEED : for reproductibility of the maze                         // example: SEED=42<br><br>
    
    All of the above parameters need to be present in the configuration file. The structure must be as follows: "PARAM=value". You can add comments, but they must start with "#" to be properly ignored. If you don't give a configuration filename in the command line, the default configuration file will be "config.txt" at the root of the repository.<br><br>
</details>
<details>
  ...
  + Wilson:<br><br>
    This algorithm works by defining a random cell as part of the final maze then from another random cell it is trying to join it by creating a random path. If the path makes a loop then it destroys the part of the path that made the loop and continues until it joins the cell randomly selected. When the cell is joined it builds the path found in the maze. Then selects another random cell that hasn't been expored yet and applies the same rule of pathfinding as before to join the existing part of the maze. It repeats until there are no more unvisited cells. It creates a maze that is always perfect, meaning that each cell is linked to another one by one only path.<br>
    I chose this algorithme, firstly because unlike some other algorithms this one doesn't have a pattern, also because it is one that is not optimized and hard to implement so I found that it would be a fun challenge to make one that works well, and finally it was fun to watch it create a maze.<br><br>

  + Recursive backtracking:<br><br>
    ...
    I chose this algoritm because I usually struggle with recursion and backtracking, so I wanted to train to better comprehend how it works. And it also is an advantageous algorithm in terms of speed and optimization.<br><br>
  ...
  + A*:<br><br>
    ...
    I chose this algorithm because I found it easy to understand and implement, and it is pretty effective and optimized. Also I liked its name haha xD<br><br>

  + Jump point search:<br><br>
    This algorithm works like the A* but it has an optimisation that makes it "walk" in the same direction until it crosses either a wall or the exit, then if it crosses a wall it calculates the cost. So that means it makes less calculation than the original A*<br><br>
</details>
<details>
...
<details>
  ...
  + Ibady:<br><br>
    ...
    - README<br><br>
  ### 2. Planning:
  We had initially planned to finish the project in about two weeks, and it took more like three weeks (mais c ok).<br><br>

  ### 3. Pros and Cons:
  We worked pretty well together, no conflicts of any sort, and we discussed how to implement things so as to disrupt what the other did as little as possible. We were also quite organised with the github branches and we used pull requests to approve/disapprove of the changes. Though we realized at some point during the project that we probably shoud have spent more time in the beginning stages of the project to establish a more defined structure and logic, because we ended up remaking a lot of things gradually as we had more ideas and problems to solve that popped up.<br><br>

  ### 4. Tools:
  We used an online visualization tool that shows how different generation and pathfinding algorithms work with animations (link in Resources section).<br><br>
</details>
<details>
  ...
  ### 2. AI Usage:
  ChatGPT was used a little bit, mostly for some explanations on how to handle the MiniLibX library, especially because of the lack of documentation for the Python version.<br><br>
</details>
