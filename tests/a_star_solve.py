...
def test_solving() -> None:
    ...
    if (
        not a_star_path
        or (
            a_star_path != path and
            len(str(a_star_path)) > len(path)
        )
    ):

        print(
            "Solving failed: path found by algorithm is "
            f"{a_star_path} instead of {path}\n"
        )
        print(
            f"length of a* path: {len(str(a_star_path))} "
            f"and length of example path: {len(path)}"
        )

    else:
        print("Solving succeeded! :)")
        print(f"Path found: {a_star_path}")
